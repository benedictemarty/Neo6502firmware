...
uint8_t RSOpen(uint8_t channel,uint16_t nameAddr) {
    ...
    std::string name((const char *)cpuMemory + nameAddr + 1,cpuMemory[nameAddr]);
    if (FIOOpenFileHandle(channel,name,FIOMODE_RDONLY) != 0) return RS_ERR_IO;
    rsChannel = channel;
    uint8_t header[8];
...
// Load resource index into page:address (pages as 3,27), at most *size bytes ; *size = bytes read.
