...
import sys,os,re
TOOLBOX_FIRST = 32                                                              # bmarty : first toolbox group (ADR-01)
...
TOOLBOX_OUT_FILE = os.path.join(ROOT_DIR , 'firmware' , 'common' , 'include' , 'data' , 'dispatch_toolbox.h')   # bmarty R22
...
class Function(object):
	...
	#
	def render(self):
		for l in self.renderLines(): print(l)
	#
	def renderLines(self):
		out = ["\t\t\tcase {0}:".format(self.funcID)]
...
class Group(object):
	...
	#
	def renderLines(self):
		out = ["\tcase {0}:".format(self.groupID)]
		if self.groupID == 4:
			out.append("\t\tMATHCommon(DPARAMS);")
		out.append("\t\tswitch (*DFUNCTION) {")
		funcs = [x for x in self.functions.keys()]
		funcs.sort()
		for f in funcs:
			out += self.functions[f].renderLines()
		out.append("\t\t\tdefault:")
		out.append("\t\t\t\tWARN_GROUP(cmd,*DFUNCTION);")
		out.append("\t\t\t\tbreak;")
		out.append("\t\t}")
		out.append("\t\tbreak;")
		return out
...
class DispatchAPI(object):
	...
	def render(self):
		...
		# bmarty R22 : groups >= TOOLBOX_FIRST (toolbox, ADR-01) are dispatched by DSPToolbox(), a
		# separate function kept in flash : DSPHandler is copied to RAM (time critical) and every
		# case added to it costs SRAM (about 300 bytes per group).
		print("switch (cmd) {")
		groups = [x for x in self.groups.keys()]
		groups.sort()
		toolbox = [k for k in groups if k >= TOOLBOX_FIRST]
		for k in groups:
			if k < TOOLBOX_FIRST: self.groups[k].render()
		if toolbox:
			for k in toolbox: print("\tcase {0}:".format(k))
			print("\t\tDSPToolbox(cBlock,memory,cmd);")
			print("\t\tbreak;")
		print("\tdefault:")
		print("\t\tWARN_GROUP(cmd,*DFUNCTION);")
		print("\t\tbreak;")
		print("}")
		out = ["//\n// This file is automatically generated (toolbox groups, dispatched by DSPToolbox in flash : R22)\n//\n\n"]
		out.append("switch (cmd) {")
		for k in toolbox:
			out += self.groups[k].renderLines()
		out.append("\tdefault:")
		out.append("\t\tWARN_GROUP(cmd,*DFUNCTION);")
		out.append("\t\tbreak;")
		out.append("}")
		with open(TOOLBOX_OUT_FILE,'w',newline='\n') as f:
			f.write("\n".join(out) + "\n")
