...
| 1 System | 16 Set Frame Interrupt | P0 = 1 actif / 0 arrêt | IRQB au début de chaque trame affichée (vsync 60 Hz), livrée/relâchée comme 1,12 ; cumulable avec le tick (non distinguables) ; arrêté par 1,0 | absente | F-10 |
| 1 System | 17 Get Frame Interrupt | — | P0 = état | absente | F-10 |
...
| 1 System | 20 Get Date Time | — | P0-1 année (1970-2099), P2 mois, P3 jour, P4 heure, P5 minute, P6 seconde, P7 source (0 non réglée = 1970-01-01 + temps écoulé, 1 logicielle, 2 PCF8563) | absente | F-14 |
| 1 System | 21 Set Date Time | P0-6 comme 1,20 | règle l'horloge logicielle et le PCF8563 s'il est présent ; erreur 1 si un champ est hors plage | absente | F-14 |
| 8 Sound | 9 Set Channel Volume | P0 = canal, P1 = volume 0-100, P2-3 = durée de la rampe (1/100 s, 0 = immédiat) | volume de la note en cours (les notes en file gardent le leur ; une nouvelle note annule la rampe) ; erreur 1 si canal inexistant ou silencieux | absente | F-12 |
| 8 Sound | 10 Get Channel Volume | P0 = canal | P1 = volume courant 0-100 (0 si silencieux) ; erreur 1 si canal inexistant | absente | F-12 |
...
| **33 Event Manager** (toolbox, ADR-01) | 1 Init Events (masque), 2 Get Next Event (P0-1 EventRecord, P2-3 masque → P4 = 1 si événement), 3 Event Available, 4 Flush Events, 5 Set Timer (0-3, période en ticks 10 ms), 6 Event Status | EventRecord 8 o : what (1 keyDown, 2 keyUp, 3 autoKey, 4 mouseDown, 5 mouseUp, 6 mouseMove fusionnés, 7 wheel, 8 timer), message, message2 (code touche / bouton, bit 7 = double clic), modifiers, X, Y | file de 32 ; aucune attente bloquante (les hôtes lisent leurs entrées entre deux appels : scruter, ou 1,12 + WAI) ; la file clavier du groupe 2 n'est pas touchée ; arrêté par DSP Reset | nouveau groupe | F-42 |
| **34 Window Manager** (toolbox, ADR-01) | 1 New Window (rect, titre, drapeaux titre/fermeture/taille → id 1-8), 2 Dispose, 3 Show, 4 Select, 5 Move, 6 Size, 7 Find Window (→ id, partie 1 contenu / 2 titre-bord / 3 fermeture / 4 taille), 8 Get Content Rect, 9 Set Title, 10 Front Window, 11 End Update, 12 Set Port | cadres, barre de titre 10 lignes, case de fermeture, case de taille dessinés par le firmware ; contenu par le 6502 sur événements **update (9)** et **activate (10)** du groupe 33, postés de l'arrière vers l'avant après chaque changement ; Set Port = clip QuickDraw sur le contenu ; End Update repeint les cadres des fenêtres au-dessus | nouveau groupe | F-43 |
| **35 Menu Manager** (toolbox, ADR-01) | 1 New Menu (descripteur en RAM 6502 → id 1-6), 2 Draw Menu Bar, 3 Menu Select (x,y → menu déroulé), 4 Menu Track (x,y), 5 Menu Track End (→ menu, item), 6 Set Item Flags, 7 Dispose Menu, 8 Get Item Flags | descripteur lu en place : titre préfixé, items = drapeaux (bit 0 inactif, bit 1 coché, bit 7 séparateur) + texte préfixé, fin `$FF` ; barre de 12 lignes en haut | suivi souris par le programme (down → Select, move → Track, up → Track End) ; rien n'est sauvé sous le menu : à la fermeture la zone est effacée et les fenêtres recouvertes reçoivent un update (`WMInvalidate`) | nouveau groupe | F-44 (menus) |
| **36 Control Manager** (toolbox, ADR-01) | 1 New Control (genre 1 bouton / 2 case / 3 radio / 4 ascenseur / 5 champ, fenêtre, rect relatif au contenu, titre ou tampon, max → id 1-24), 2 Dispose, 3 Draw Controls (fenêtre), 4 Draw Control, 5/6 Set/Get Value, 7 Set Flags (inactif), 8 Find Control (→ id, partie : corps / flèches / pages / curseur), 9 Track Control (x, y, phase down/move/up → agi), 10 Key Control (champ : saisie, 8 = effacer) | titres et tampons lus en place en RAM 6502 ; contrôles détruits avec leur fenêtre ; radios non groupés (le programme décoche les autres) | nouveau groupe | F-44 (contrôles) |
| **37 Dialog Manager** (toolbox, ADR-01) | 1 New Dialog (descripteur en RAM 6502 → id 1-2, fenêtre), 2 Dispose Dialog, 3 Dialog Event (EventRecord → item qui a agi, consommé), 4 Get Dialog Item (→ contrôle, genre), 5 Draw Dialog, 6 Alert (message, boutons OK / OK-Cancel / Yes-No → id, fenêtre) | descripteur lu en place : rect écran, drapeaux (bit 0 titre), titre préfixé, items (genre 1-5 contrôles / 6 texte statique, drapeaux inactif / défaut / annulation, rect relatif, max, texte ou tampon préfixé), fin 0 | modal sans appel bloquant : le programme passe chaque événement du groupe 33 à Dialog Event (souris/clavier consommés ; update du dialogue redessiné ; les autres rendus au programme) ; Return = item par défaut (anneau), Escape = annulation, Tab = champ suivant ; alertes centrées, titres OK/Cancel/Yes/No en flash | nouveau groupe | F-45 |
| **38 Resource Manager** (toolbox, ADR-01) | 1 Open Resource File (canal, nom), 2 Close, 3 Count, 4 Find (type 4 car., id → n°), 5 Get Info (→ type, id), 6 Get Size (32 bits), 7 Load (n°, page, adresse, max → lus), 8 Use Font (Load + 32,15) | fichier **NR1** (`docs-bmarty/tools/mkres.py`) : en-tête 8 o, entrées de 16 o (type, id, décalage, taille), données ; pages de destination = celles de 3,27 | un fichier ouvert à la fois, table relue sur le stockage (rien en SRAM) ; erreurs 1 paramètre / pas un NR1, 2 stockage, 3 introuvable | nouveau groupe | F-46 |
| 1,6 Locale, 2,5 Define Character, 2,6 Write Character, 5,x Draw Text, 32,16/17 | inchangés | **Latin-1** (F-17) : `$A0-$BF` glyphes en flash (° ² £ § µ ¤ ¨ « » ¿ …), `$C0-$FF` police utilisateur initialisée aux lettres accentuées au reset (2,5 les remplace) ; la console affiche `$A0-$FF` (avant : `$C0-$FF` seulement) ; `$80-$9F` restent des codes de couleur | `fr.locale` = AZERTY PC en Latin-1 (`FM` = Apple) ; codes 160-255 admis dans les `.locale` | extension | F-17 |
