...
| ID | P | Story | Origine | État |
|----|---|-------|---------|------|
| F-10 | P1 | **IRQ périodique / vsync** : le RP2040 déclenche l'IRQ du 65C02 (broche IRQB, déjà câblée) à 50/60 Hz ou sur trame, activable par API (nouvelle fonction groupe 1), avec acquittement ; vecteur `$FFFE` laissé au programme. | bbc US-16, oric2 | **DONE** 2026-09-18 : **1,16 Set Frame Interrupt** / **1,17 Get Frame Interrupt** — IRQB au début de chaque trame depuis le callback scanline de core 1 (`_scanline_callback`, `frameIrqOn`, ~10 cycles ARM, hors boucle bus), même relâchement que le tick (lecture de `$FFFF`) ; `neo` : une IRQ par trame émulée ; test `frameirq.asm` (60 trames comptées, arrêt vérifié) ; USB/SDCARD compilés, **non testé sur carte** (le 50/60 Hz périodique était déjà couvert par F-60) |
...
| F-12 | P2 | **Son : volume instantané par canal** et/ou enveloppe simple (attaque/relâchement) en complément de 8,7. | bbc US-14 | **DONE** 2026-09-17 : **8,9 Set Channel Volume** (volume 0-100 de la note en cours, atteint en P2-3 centièmes de seconde par rampe linéaire au tick 50 Hz, 0 = immédiat ; sans redémarrage de l'onde, `SNDSetCreatorVolume`) et **8,10 Get Channel Volume** ; test `sndvol.asm` (`neo`) ; USB compilé, non testé sur carte. Attaque/relâchement = deux appels 8,9 (ou 8,7 puis 8,9) |
...
| F-14 | P2 | **Date/heure** : API lecture/écriture d'une horloge (RTC PCF8563 sur I2C si présent, sinon compteur), horodatage FAT. | manques | **DONE (API)** 2026-09-17 : **1,20 Get Date Time** / **1,21 Set Date Time** (`clock.cpp` : PCF8563 à `$51` sondé sur le bus I2C UEXT, sinon horloge logicielle sur le timer 100 Hz ; source dans P7) ; `neo` modélise le PCF8563 sur l'heure de l'hôte ; test `datetime.asm` ; USB compilé, **I2C/RTC non testés sur carte** ; **horodatage FAT** : `HWClockSet` programme la RTC du RP2040 (`hardware_rtc`) au réglage et à la détection du PCF8563 → `get_fattime()` de la bibliothèque SD (`FF_FS_NORTC = 0`) ; stockage USB : FatFs de TinyUSB compilé avec `FF_FS_NORTC = 1`, pas d'horodatage (dépendance FetchContent) |
...
| F-17 | P2 | **Locale FR** (demande bmarty 2026-09-18). | Télémon / Neo6502kbd | **DONE** 2026-09-18 (`feat/locale-fr`) : `fr.locale` AZERTY PC en codes Latin-1 (l'amont, disposition Apple, devient `fm.locale`) ; glyphes `$A0-$BF` en flash et `$C0-$FF` par défaut dans la police utilisateur (2,5 les remplace) ; console, Draw Text et QuickDraw ; `latin1.asm` ; libellés toolbox (c) non faits, non demandés |
...
| F-57 | P3 | Police 9×14 et 8×8 chargeables (fichiers), jeu de caractères graphiques ; modes texte 80×43 (9×8) sur Hercules. | Télémon | **retiré** 2026-09-18 (décision bmarty, R22) : le mode 3 (80×43) livré par l'autre session a été reverti (`13f6042`) pour rendre la SRAM au tas |
...
| F-41 | P2 | **QuickDraw (32)** : port courant + clipping, rectangles, lignes, motifs, `CopyBits` sur le blitter, fontes proportionnelles (format + outil PC de conversion), `DrawString/TextWidth` ; captures golden Phosphoneo. | | **DONE** 2026-09-17 : incr. 1 (`feat/quickdraw-lines` : 32,11-32,14 lignes, motifs, Copy Bits ; `quickdraw2.asm`) + incr. 2 (`feat/quickdraw-fonts` : format NF1, 32,15-32,18 Set Font / Draw String / Text Width / Get Font Info, `tools/mkfont.py`, `qdfont.asm`) ; identique neo / Phosphoneo / co-sim, non testé sur carte. Régions rectangulaires : reportées (pas de besoin) |
| F-42 | P2 | **Event Manager (33)** : file unifiée clavier/souris/fenêtre/timer, `GetNextEvent`, `WaitNextEvent` (timeout ; IRQ F-10 quand disponible). | | **DONE** 2026-09-17 (`feat/event-manager`) : 33,1-33,6, clavier/souris/timers ; **pas de `WaitNextEvent` bloquant** (les hôtes lisent leurs entrées hors appel API ; sommeil par 1,12 + WAI) ; événements fenêtre → F-43 ; `events.asm`, `evtimer.asm` ; non testé sur carte |
| F-43 | P2 | **Window Manager (34)** : fenêtres, ordre Z, cadres dessinés par le firmware, invalidation/`update`, drag/size, `FindWindow`. | | **DONE** 2026-09-17 (`feat/window-manager`) : 34,1-34,12, événements update/activate (33), drag/size par le programme (Move/Size + Find Window) ; contenu non conservé (redessin sur update) ; `wm.asm` ; non testé sur carte |
| F-44 | P3 | **Menu (35) et Control (36) Managers**. | | **DONE** 2026-09-18 : Menu Manager (`feat/menu-manager` : 35,1-35,8, `menu.asm`) et Control Manager (`feat/control-manager` : 36,1-36,10, `ctl.asm`) ; non testé sur carte |
| F-45 | P3 | **Dialog Manager (37)**, descriptions en RAM 6502. | | **DONE** 2026-09-18 (`feat/dialog-manager`) : 37,1-6 New/Dispose/Dialog Event/Get Dialog Item/Draw/Alert, descripteur lu en place, modal sans appel bloquant, alertes centrées ; `dlg.asm` identique neo / Phosphoneo / co-sim ; USB 3 280 o de tas ; **non testé sur carte** |
